...
def get_cosine_schedule_with_warmup(
    optimizer: Optimizer, num_warmup_steps: int, num_training_steps: int, num_cycles: float = 0.5, last_epoch: int = -1, eta_min=1e-6,opt=None
):
    """
    ...
    def lr_lambda(current_step):
        if (opt is not None) and (opt.KDLoss) and (current_step*opt.batch_size)//opt.epoch_sam_num < opt.KD_epoch:
            return 3
