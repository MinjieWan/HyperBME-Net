...
def get_flops_pytorch(model, input_res,
    ...
    if input_constructor:
        ...
        try:
            if type(input_res) is tuple:
                batch = torch.ones(()).new_empty((1, *input_res),
                                                 dtype=next(flops_model.parameters()).dtype,
                                                 device=next(flops_model.parameters()).device)
                _ = flops_model(batch)
            elif type(input_res) is list:
                batch = (torch.ones(()).new_empty([1, *input_res[i]],
                                                 dtype=next(flops_model.parameters()).dtype,
                                                 device=next(flops_model.parameters()).device) for i in range(len(input_res)))
                _ = flops_model(*batch)
            ...
            if type(input_res) is tuple:
                ...
                batch = (torch.ones(()).new_empty([1, *input_res[i]]) for i in range(len(input_res)))
                _ = flops_model(*batch)
